...
def add_product():
    if request.method == 'POST':
        try:
            ...
            variant_urls = request.form.getlist('variant_urls[]')
            variant_images = request.files.getlist('variant_images[]')

            for i in range(len(variant_names)):
                variant_image = None
                if variant_images[i] and allowed_file(variant_images[i].filename):
                    upload_result = cloudinary.uploader.upload(variant_images[i])
                    variant_image = upload_result['secure_url']
...
def edit_product(product_id):
    ...
    if request.method == 'POST':
        try:
            ...
            product.discounted_price = request.form['discounted_price']
            ...
            for vid, name, value, url, image in zip(variant_ids, variant_names, variant_values, variant_urls, variant_images):
                if name and value:
                    if vid and vid != 'null' and vid in existing_variants:
                        ...
                        if image and allowed_file(image.filename):
                            ...
                            variant.image_file = upload_result['secure_url']
                    else:
                        # Thêm biến thể mới
                        variant_image = None
                        if image and allowed_file(image.filename):
